...
// Code generated by github.com/af-afk/stylus version 962d3b81f7af684a262e9d49dfe30b2ba1b6bfa3
...
func userEntrypoint(cdlen int32) int32 {
	...
	if bytes.Equal(args[:4], []byte{16,3,226,210,}) {
		...
		rd = append(rd, stylus.BigToInt256Bytes(rdAdd)...)
	}
	if bytes.Equal(args[:4], []byte{250,59,214,197,}) {
		...
		rd = append(rd, stylus.BigToInt256Bytes(rdSub)...)
	}
	if bytes.Equal(args[:4], []byte{6,102,26,189,}) {
		...
		rd = append(rd, stylus.BigToInt256Bytes(rdCount)...)
	}
	if bytes.Equal(args[:4], []byte{16,3,226,210,}) {
		var err error
		if len(args) != 4 + 32 {
